					...
					if(runningTimers[j].time > 0){
						...
						if(namingPrompt.split(':').join('').split(' ').join('-') == cmd){
							...
							if(members[messagerIndex].skillExp[3] >= 150){
								...
								channelID.send('You\'ve become more familiar with this industry. Your skill in this industry increases to level ' + members[messagerIndex].skillSet[3] + '!');
							}
							var pay = (members[messagerIndex].jobAt.wage * (members[messagerIndex].skillSet[3]));
							
							members[messagerIndex].balance += pay;
							channelID.send('Nice job! You\'re paid $' + pay + ' for your hard work. (' + members[messagerIndex].skillExp[3] + '/150 XP)');
					...
					if(runningTimers[j].time > 0){
						if(translationPrompt == cmd){
							...
							if(members[messagerIndex].skillExp[2] >= 150){
								...
								channelID.send('You\'ve become more familiar with this industry. Your skill in this industry increases to level ' + members[messagerIndex].skillSet[2] + '!');
							}
							var pay = (members[messagerIndex].jobAt.wage * (members[messagerIndex].skillSet[2]));
							members[messagerIndex].balance += pay;
							runningTimers[j].feed.send('Nice job! You\'re paid $' + pay + ' for your hard work. (' + members[messagerIndex].skillExp[2] + '/150 XP)');
					...
					if(runningTimers[j].time > 0){
						if(countingPrompt == cmd){
							...
							if(members[messagerIndex].skillExp[1] >= 150){
								...
								channelID.send('You\'ve become more familiar with this industry. Your skill in this industry increases to level ' + members[messagerIndex].skillSet[1] + '!');
							}
							var pay = (members[messagerIndex].jobAt.wage * (members[messagerIndex].skillSet[1]));
							members[messagerIndex].balance += pay;
							runningTimers[j].feed.send('Nice job! You\'re paid $' + pay + ' for your hard work. (' + members[messagerIndex].skillExp[1] + '/150 XP)');
					...
					if(runningTimers[j].time > 0){
						if(additionPrompt == cmd){
							...
							if(members[messagerIndex].skillExp[0] >= 150){
								...
								channelID.send('You\'ve become more familiar with this industry. Your skill in this industry increases to level ' + members[messagerIndex].skillSet[0] + '!');
							}
							var pay = (members[messagerIndex].jobAt.wage * (members[messagerIndex].skillSet[0]));
							members[messagerIndex].balance += pay;
							runningTimers[j].feed.send('Nice job! You\'re paid $' + pay + ' for your hard work. (' + members[messagerIndex].skillExp[0] + '/150 XP)');
        ...
        switch(cmd) {
			...
			case 'bid':
				...
				if(args.length < 2){		
					...
					var currBid = checkCurrentBid();
					if(currBid != -1){
						channelID.send('If the auction closed now, ' + currBid.bidder + ' would get the ' + currAuction.amount + ' ' + currAuction.item.name + '(s) for ' + currAuction.currBid + '.');
					}else{
			...
			case 'auction':
				...
				for(l = 0; l < members[messagerIndex].inventory.length; l++){
					if(itemName == members[messagerIndex].inventory[l].name){
						if(members[messagerIndex].inventory[l].num >= amount){
							...
							channelID.send('Putting up '+ amount + ' ' + members[messagerIndex].inventory[l].name +'.');
							dontHave = false;
						}
						break;
					}
				...
				channelID.send('Starting auction for ' + amount + ' ' + itemName + '(s)! Bidding starts at ' + startBid + '! @' + members[messagerIndex].memberName + ', type ' + prefix + 'close to close the auction and complete the transaction.');
            ...
            case 'help':
				...
				break;
			case 'update':
				channelID.send(getUpdateMessage(user));
				break;
			case 'whoami':
			...
			case 'balance':
				...
				for(j = 0; j < members[messagerIndex].inventory.length; j++){
					if(members[messagerIndex].inventory[j].num > 0){
...
client.login('NTU0ODM0NDc4MzEzMjQyNjI1.D2icdQ.ET3BbLbn1mI9OX_I60DYCQjWkGE');